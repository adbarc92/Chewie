...
const isAdminRole = req => new Promise((resolve, reject) => {
  try {
    app.models.user.isAdminRole(req.user.id, (err, status) => {
      if (err) reject(err);
      const loggedUser = req.user;
      loggedUser.isAdmin = status;
      loggedUser.token = req.accessToken.id;
      resolve(loggedUser);
    });
  } catch (err) {
    console.log(err);
...
app.middleware('session:before', cookieParser(process.env.SECRET_COOKIE));
...
// middleware to add branding information so it can be later passed to ejs templates.
app.use((req, res, next) => {
  req.brand = brands[req.headers.host] || brands.fallback;
  next();
});
...
app.post('/signin', (req, res) => {
  ...
  req.login(user, (err) => {
    if (err) {
      winston.log('error', `ERROR LOGIN IN::: ${err}`);
...
app.post('/signup', (req, res) => {
  ...
  User.create(newUser, (err, user) => {
    if (err) {
      winston.log('error', `ERROR CREATING USER::: ${err}`);
      req.flash('error', err.message);
      return res.redirect('back');
    }
    ...
    return user.verify(options, (error, response, next) => {
      ...
      req.login(user, (errLogin) => {
        if (errLogin) {
          winston.log('error', `ERROR LOGIN IN::: ${errLogin}`);
          req.flash('error', err.message);
          return res.redirect('back');
        }
        return res.redirect('/auth/account');
...
app.get('/login', (req, res) => {
  res.render('pages/login', { brand: req.brand });
});

app.get('/signup', (req, res) => {
  // if we have an admin hostname then show the admin login.
  if (req.brand.admin) {
    res.render('pages/signup', { brand: req.brand });
  } else {
    res.render('/', { brand: req.brand });
  }
});

app.get('/', (req, res) => {
  res.render('pages/index', { brand: req.brand });
});

app.start = () => app.listen(() => {
  app.emit('started');
  const baseUrl = app.get('url').replace(/\/$/, '');
  winston.log('info', `CHEWIE SERVER HAS STARTED CONFIG: {baseUrl:${baseUrl}}`);
...
if (require.main === module) {
  ...
  app.io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      winston.log('error', 'socket disconnected: ', socket);
    });
  });
}
